...
# Fit a K-NN model
k_value = 6
knn_model <- knn(train = dataTrainAll, test = dataTestAll, cl = dataTrainAll$SalePrice, k = k_value)
predictions <- knn(train = dataTrainAll, test = dataTest, cl = dataTrainAll$SalePrice, k = k_value)
#accuracy <- mean(predictions == dataTrainAll$SalePrice)